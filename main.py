...
output_path = config["MISC"]['output_directory']
...
for i in range(len(response_folders.json()["result"]["folders"])):
    print(str(i+1) + ". " + response_folders.json()["result"]["folders"][i]["fld_id"] + " " + response_folders.json()["result"]["folders"][i]
    ["name"])

FLD_NUM = int(input())
FLD_ID = response_folders.json()["result"]["folders"][FLD_NUM-1]["fld_id"]
...
with open(output_path+'\output.xml', 'w', encoding='utf-8') as file:
    file.write(xmlka)
